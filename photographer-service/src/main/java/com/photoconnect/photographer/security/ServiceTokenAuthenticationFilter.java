...
/**
 * Verifies service-to-service JWTs on incoming requests.
 *
 * <p>This filter is ordered BEFORE {@link GatewayAuthenticationFilter}. If
 * a {@code Bearer} token is present and verifies as {@code typ=service}, we
 * establish the authentication and the gateway-headers filter is a no-op for
 * this request (it finds the context already populated and bails).</p>
 *
 * <h2>Granted authorities</h2>
 * <ul>
 *   <li>{@code ROLE_SERVICE} — broadly distinguishes "another service called us"
 *       from "a user called us through the gateway".</li>
 *   <li>{@code SCOPE_<value>} — one authority per scope listed in the JWT's
 *       {@code scope} claim (comma-separated). Endpoint guards use
 *       {@code hasAuthority('SCOPE_photographer-read')}.</li>
 * </ul>
 *
 * <h2>Failure mode</h2>
 * <p>If a Bearer token is present but invalid (bad signature, wrong issuer/audience,
 * expired, wrong {@code typ}), we leave the SecurityContext empty and let the
 * authorization rules return 401. We do NOT throw — that would short-circuit
 * the filter chain and skip {@link GatewayAuthenticationFilter}, breaking the
 * case where a forwarded user request somehow carries a stray Authorization
 * header.</p>
 */
@Slf4j
@Component
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            if (!TYP_SERVICE.equals(typ)) {
                // Not a service token — could be a forwarded user access token.
                // Let the gateway-headers filter (which runs next) handle the
                // user case. Don't reject here.
